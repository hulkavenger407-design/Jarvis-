...
from kernel.interfaces import IAgentRuntime
...
from .interfaces import Agent, AgentFactory
from .models import (
    AgentContext,
    AgentExecution,
    AgentHandle,
...
class AgentRuntime(KernelSubsystem, IAgentRuntime):
    """
    ...
    def register_agent_sync(
        self, agent: Agent, config: dict[str, Any] | None = None
    ) -> AgentHandle:
    ...
    def unregister_agent_sync(self, agent_id: str) -> None:
    ...
    async def create_agent(self, factory_name: str, config: dict[str, Any]) -> str:
        ...
        handle = self.register_agent_sync(agent, config)
    ...
    async def destroy_agent(self, agent_id: str) -> None:
        ...
        self.unregister_agent_sync(agent_id)
    ...
    def list_agents_sync(self) -> list[AgentHandle]:
    ...
    async def execute(self, agent_id: str, capability: str, payload: Any) -> Any:
        return await self.execute_agent(agent_id)

    async def register_agent(self, agent: Any) -> None:
        self.register_agent_sync(agent)

    async def unregister_agent(self, agent_id: str) -> None:
        await self.destroy_agent(agent_id)


    def get_agent_sync(self, agent_id: str) -> AgentHandle:
        return self._get_handle(agent_id)

    def get_agent(self, agent_id: str) -> Any:
        return self.get_agent_sync(agent_id)
